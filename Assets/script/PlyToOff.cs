...
public class PlyToOff : MonoBehaviour
{

    public Material material;
    public string fileName;
    public int i, numVertex, numTriangle;
    ...
    bool ReadFile()
    {
        ...
        try
        {
            ...
            {
                sw.WriteLine("OFF");
                sw.WriteLine(numVertex + " " + numTriangle + " " + 0);
                foreach(var vert in sverts)
                {
                    sw.WriteLine(vert.x + " " + vert.y + " " + vert.z);

                }
                for (i = 0; i < numTriangle; i++)

                {
                    sw.WriteLine("3" + " " + triangles[3*i] + " " + triangles[3*i+1] + " " + triangles[3*i+2]);
                }
